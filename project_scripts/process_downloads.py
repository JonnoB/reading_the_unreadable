...
folders = [f for f in os.listdir(path) if os.path.isdir(os.path.join(path, f)) and f != "dataframes"]
...
for folder in folders:
    ...
    if not os.path.exists(post_processed_output):
        ...
        print(f"Skipping {folder} post processed dataframe as {output_parquet} already exists")
    if not os.path.exists(article_output_path):
        returned_docs = pd.read_parquet(post_processed_output)

        articles_df = process_documents(returned_docs, batch_size = 1, parallel= True, n_jobs = 1)
