...
def _():
    ...
    import pandas as pd
    import pandas as np
    from function_modules.helper_functions_class import create_genre_prompt, create_iptc_prompt
...
def _(classifications_dict_to_df, data_set, process_dictionary_responses):
    ...
    text_class_df['issue_id'] = text_class_df['custom_id'].str.split("_page", n = 1, expand = True)[0]
    text_class_df = text_class_df.dropna(subset=['class_code'])
    text_class_df['class_code']  = text_class_df['class_code'].astype(int)
    sampled_issue_ids_sample_method = text_class_df['issue_id'].drop_duplicates().sample(frac=0.8, random_state=1852)

    # Step 2: Create a new boolean column 'train' using the sampled issue_ids
    text_class_df['is_train'] = text_class_df['issue_id'].isin(sampled_issue_ids_sample_method)
...
def _(class_to_int, pd, text_class_df):
    test_set_preds = pd.read_parquet('data/classification/predicted_text_type_testset.parquet').merge(text_class_df[['bbox_uid', 'text_class2', 'is_train']])
    ...
    test_set_preds['predicted_class'] = test_set_preds['predicted_codes'].map(int_to_class)
...
def _(
    ...
    print(classification_report(test_set_preds['text_class2'], test_set_preds['predicted_class']))

    # Create and plot confusion matrix
    cm = confusion_matrix(test_set_preds['text_class2'], test_set_preds['predicted_class'])
    ...
    misclassified = test_set_preds[test_set_preds['text_class2'] != test_set_preds['predicted_class']]
...
def _(os, pd):
    ...
    text_class_temp = pd.read_parquet(os.path.join(_text_classification_folder, all_text_classification_preds[1]))
    return all_text_classification_preds, text_class_temp


@app.cell
def _(os, pd):
    _output_file = os.path.join('data/classification', 'IPTC_class_training_set.parquet')
    ...
    if os.path.exists(_output_file):
        ...
        for _periodical in _all_data_parquets:
            ...
            _df = _df.loc[_df['class'].isin(['title', 'text']) & 
...
def _(client, download_processed_jobs, os, pd):
    ...
    if os.path.exists(_jobs_file):
        ...
        if os.path.exists(_expected_jsonl):
            ...
            download_processed_jobs(client, jobs_file=_jobs_file, output_dir=_output_dir, log_file=_log_file)
    else:
...
def _(classifications_dict_to_df, pd, process_dictionary_responses):
    classified_IPTC_type =process_dictionary_responses( 'data/classification/silver_data/text_IPTC/text_IPTC_classification.jsonl')


    IPTC_class_df_raw = classifications_dict_to_df(classified_IPTC_type)[['custom_id', 'class_code']]
...
def _(IPTC_class_df, IPTC_data_set):
    IPTC_class_df_out = IPTC_class_df.merge(IPTC_data_set, left_on = 'custom_id', right_on=['bbox_uid']).drop(columns = (['bbox_uid',  'NA', 'sport', 'weather', 'environment']))


    IPTC_class_df_out['issue_id'] = IPTC_class_df_out['custom_id'].str.split("_page", n = 1, expand = True)[0]
    _sampled_issue_ids_sample_method = IPTC_class_df_out['issue_id'].drop_duplicates().sample(frac=0.8, random_state=1852)

    # Step 2: Create a new boolean column 'train' using the sampled issue_ids
    IPTC_class_df_out['is_train'] = IPTC_class_df_out['issue_id'].isin(_sampled_issue_ids_sample_method)
    return (IPTC_class_df_out,)


@app.cell
def _(IPTC_class_df_raw, IPTC_data_set):
    IPTC_class_df_for_train = IPTC_class_df_raw.merge(IPTC_data_set, left_on = 'custom_id', right_on=['bbox_uid']).drop(columns = ['bbox_uid'])

    IPTC_class_df_for_train['issue_id'] = IPTC_class_df_for_train['custom_id'].str.split("_page", n = 1, expand = True)[0]
    _sampled_issue_ids_sample_method = IPTC_class_df_for_train['issue_id'].drop_duplicates().sample(frac=0.8, random_state=1852)

    # Step 2: Create a new boolean column 'train' using the sampled issue_ids
    IPTC_class_df_for_train['is_train'] = IPTC_class_df_for_train['issue_id'].isin(_sampled_issue_ids_sample_method)



    IPTC_class_df_for_train.to_parquet('data/classification/silver_data/silver_IPTC_class.parquet')
...
def _(iptc_mapping, pd, text_class_df):
    test_set_preds_IPTC = pd.read_parquet('data/classification/predicted_IPTC_type_testset.parquet').merge(text_class_df[['bbox_uid', 'text_class2', 'is_train']])
...
def _(
    ...
    IPTC_gt_df = IPTC_class_df.sort_values('custom_id').loc[IPTC_class_df['custom_id'].isin(test_set_preds_IPTC['bbox_uid'])][sorted(iptc_mapping.values())]


    IPTC_preds_df = test_set_preds_IPTC.sort_values('bbox_uid')[sorted(iptc_mapping.values())]
