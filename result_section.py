...
def _():
    ...
    periodical_name_mapping = {'English_Womans_Journal_issue_PDF_files.parquet':'English Woman’s Journal',
...
def _(
    ...
    results_df['content_paragraphs'] = results_df['content'].apply(lambda x: len([p for p in x.split('\n\n') if p.strip()]))
    results_df['content_lines'] = results_df['content'].apply(lambda x: len([l for l in x.split('\n') if l.strip()]))
    results_df['GT_paragraphs'] = results_df['GT'].apply(lambda x: len([p for p in x.split('\n\n') if p.strip()]))
    results_df['GT_lines'] = results_df['GT'].apply(lambda x: len([l for l in x.split('\n') if l.strip()]))
...
def _(np, pd, results_df):
    ...
    percentage_results = results_df.loc[results_df['dataset']!='BLN'].groupby(['model']).apply(
        lambda x: pd.Series({
            'paragraph_mape': mean_absolute_percentage_error(x['GT_paragraphs'], x['content_paragraphs']) * 100,
            'line_mape': mean_absolute_percentage_error(x['GT_lines'], x['content_lines']) * 100,
            # For median percentage error
            'paragraph_medape': np.median(np.abs((x['GT_paragraphs'] - x['content_paragraphs']) / x['GT_paragraphs'])) * 100,
            'line_medape': np.median(np.abs((x['GT_lines'] - x['content_lines']) / x['GT_lines'])) * 100
        })
    ).round(2)
...
def _(results_df, sns):
    sns.kdeplot(results_df.loc[(results_df['dataset']!='BLN') & (results_df['model']=='pixtral')], )
    return
...
def _(os, pd, pq, tqdm):
    ...
    if not os.path.exists(data_counts_path):
        ...
        for periodical in all_data_parquets:
            ...
            for batch in tqdm(parquet_file.iter_batches(batch_size=batch_size, columns=['page_id', 'box_page_id', 'content', 'class', 'prompt_tokens', 'completion_tokens', 'total_tokens']), desc=f'Processing {periodical}'):
...
def _(dataset_folder, os, pd):
    temp = pd.read_parquet(os.path.join(dataset_folder, "Tomahawk_issue_PDF_files.parquet"))
    return (temp,)


@app.cell
def _(data_counts_df):
    data_counts_df[['char_count','word_count', 'line_count', 'paragraph_count', 'prompt_tokens', 'completion_tokens', 'total_tokens']].sum()
    return
...
def _(data_counts_df, periodical_name_mapping):
    ...
    # Second DataFrame
    _df2 = data_counts_df.groupby(['periodical', 'class']).size().reset_index(name='count').pivot(index='periodical', columns='class', values='count')

    # Merge the two DataFrames
    data_info_df = _df1.merge(_df2, left_index=True, right_index=True).reset_index()

    data_info_df = (data_info_df
                    .rename(columns = {'issue':'Issues', 
                                     'page_id':'Pages', 
                                     'figure':'Figures', 
                                     'table':'Tables', 
                                     "text":'Text',
...
def _(data_counts_df, periodical_name_mapping):
    ...
    (data_counts_df
     .assign(periodical=data_counts_df['periodical'].map(periodical_name_mapping))
     .groupby('periodical')[['char_count','word_count', 'line_count', 'paragraph_count']]
     .sum()
    )
    return
