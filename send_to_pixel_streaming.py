...
def _():
    ...
    CLD_bbox_df = pd.read_parquet(
        "data/periodical_bboxes/post_process/Leader_issue_PDF_files_1040.parquet"
    )
    # sub-select 1 or two issues from EWJ

    CLD_sub = CLD_bbox_df.loc[
        CLD_bbox_df["filename"].isin(
            ["CLD-1850-04-20_page_10.png", "CLD-1850-04-20_page_11.png"]
        )
    ]
    return (
        CLD,
...
def _(
    ...
    CLD_content2 = process_image_with_api(
        _target_image["image"],
    ...
    CLD_content2 = convert_returned_streaming_to_dataframe(
        CLD_content2, id=_image_id, custom_id=_image_id
    )
    return (CLD_content2,)
...
def _(assign_columns, basic_box_data, create_reading_order, np, pd):
    ...
    with open(
        "data/converted/Export  project - NCSE_bbox_testset - 1_11_2025.ndjson", "r"
    ) as file:
        for line in file:
            if line.strip():  # Skip empty lines
                ...
                boxes = json_obj["projects"][project_key]["labels"][0]["annotations"][
                    "objects"
                ]
                ...
                for box in boxes:
                    ...
                    row.update(
                        {"class": box["name"], "x1": x1, "y1": y1, "x2": x2, "y2": y2}
                    )
    ...
    df2["box_page_id"] = (
        "B"
        + df2["page_block"].astype(str)
        + "C"
        + df2["column_number"].astype(str)
        + "R"
        + df2["reading_order"].astype(str)
    )
...
def _(mo):
    mo.md(
        """The below block only runs if the files have not already been created as it takes about 30 mins and costs money"""
    )
    return
...
def _(
    ...
    if not os.path.exists(_save_path):
        ...
        for _image_id, _image_data in tqdm(_encoded_images.items()):
            ...
            _response = process_image_with_api(
                _image_data["image"], _prompt, model="mistral/pixtral-12b-2409"
            )

            # Create a dictionary for each iteration
            _extracted_data = convert_returned_streaming_to_dataframe(
                _response, id=_image_id, custom_id=_image_id
            )
...
def _(process_image_with_api):
    ...
    example_image = Image.open(
        "data/example_for_paper/NS2-1843-04-01_page_4_excerpt.png"
    )

    example_image = deskew_image(example_image)

    _key, _encoded_example_dict = encode_pil_image(
        example_image, "NS2-1843-04-01_page_4", "B0C1R0", 0, image_class
    )

    returned_example = process_image_with_api(
        _encoded_example_dict["image"], _prompt, model="mistral/pixtral-12b-2409"
    )
    return (
        Image,
        deskew_image,
