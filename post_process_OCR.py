...
def _():
    ...
    periodical_mapping = pd.DataFrame(
        {
            "periodical": [
                "TEC",
                "FTEC",
    ...
    path_mapping = {
        "CLD": os.path.join(
            image_path, "converted/all_files_png_120/Leader_issue_PDF_files"
        ),
        "EWJ": os.path.join(
            image_path,
            "converted/all_files_png_120/English_Womans_Journal_issue_PDF_files",
        ),
        "MRP": os.path.join(
            image_path, "all_files_png_120/Monthly_Repository_issue_PDF_files"
        ),
        "TTW": os.path.join(
            image_path, "converted/all_files_png_120/Tomahawk_issue_PDF_files"
        ),
        "TEC": os.path.join(
            image_path,
            "converted/all_files_png_120/Publishers_Circular_issue_PDF_files",
        ),
        "NS": os.path.join(
            image_path, "converted/all_files_png_200/Northern_Star_issue_PDF_files"
        ),
    }
    ...
    raw_bboxes_df = [
        pd.read_parquet(os.path.join(raw_bbox_path, _file_path))
        for _file_path in os.listdir(raw_bbox_path)
    ]
    raw_bboxes_df = pd.concat(raw_bboxes_df, ignore_index=True)

    bboxes_df = [
        pd.read_parquet(os.path.join(bbox_path, _file_path))
        for _file_path in os.listdir(bbox_path)
    ]
...
def _(
    ...
    def plot_boxes_on_image2(
        ...
        unique_classes = [
            cls for cls in df["class"].unique() if cls not in fixed_colors
        ]
        ...
        if show_reading_order and "reading_order" in df.columns:
            ...
            for i in range(len(df_sorted) - 1):
                start = (df_sorted.iloc[i]["center_x"], df_sorted.iloc[i]["center_y"])
                    ...
                    df_sorted.iloc[i + 1]["center_x"],
                    df_sorted.iloc[i + 1]["center_y"],
                )
    ...
    def pad_to_match_height(image1, image2):
        ...
        if image1.shape[0] < max_height:
            ...
            padding = np.full(
                (pad_height, image1.shape[1], image1.shape[2]), 255, dtype=np.uint8
            )
        ...
        if image2.shape[0] < max_height:
            ...
            padding = np.full(
                (pad_height, image2.shape[1], image2.shape[2]), 255, dtype=np.uint8
            )
    ...
    for _filename in test2["filename"].unique():
        _periodical_code = periodical_mapping.loc[
            periodical_mapping["periodical"] == _filename.split("-")[0],
            "periodical_code",
        ].iloc[0]
        ...
        fig1 = plot_boxes_on_image2(
            bboxes_df[bboxes_df["filename"] == _filename],
        ...
        _temp["class"] = np.where(
            _temp["class"] == "plain text", "text", _temp["class"]
        )
        _temp["class"] = np.where(
            _temp["class"].isin(["text", "figure", "table"]), _temp["class"], "other"
        )
        fig2 = plot_boxes_on_image2(
            _temp,
            image_path=os.path.join(_image_path, _filename),
            show_reading_order=True,
        )
        ...
        _image1_padded, _image2_padded = pad_to_match_height(
            _image1_trimmed, _image2_trimmed
        )
...
def _(bboxes_df, os, periodical_mapping, plot_boxes_on_image, plt):
    ...
    for _target_page in os.listdir("data/converted/ncse_bbox_test_set_hyphen"):
        _periodical_code = periodical_mapping.loc[
            periodical_mapping["periodical"] == _target_page.split("-")[0],
            "periodical_code",
        ].iloc[0]
        #  Identify image folder using the periodical ID
        _image_path = "data/converted/ncse_bbox_test_set_hyphen"

        _image = plot_boxes_on_image(
            bboxes_df[bboxes_df["filename"] == _target_page],
            image_path=os.path.join(_image_path, _target_page),
            show_reading_order=True,
        )
...
def _(
    ...
    returned_docs = pd.read_parquet(
        os.path.join(returned_docs_folder, "Leader_issue_PDF_files.parquet")
    )
    ...
    returned_docs.loc[
        (returned_docs["completion_tokens"] > 50) & (returned_docs["class"] == "title"),
        "class",
    ] = "text"
...
@app.cell
def _(temp5):
    temp5.sort_values(["page_number", "reading_order"])[
        ["page_number", "reading_order", "class", "content", "title"]
    ]
    return
...
def _(os, pd, periodical_mapping, sns):
    ...
    parquet_files = [
        f for f in os.listdir(returned_docs_folder2) if f.endswith(".parquet")
    ]
    ...
    _combined_df = pd.concat(dataframes, ignore_index=True).merge(
        periodical_mapping, on="periodical", how="left"
    )

    sns.displot(
        data=_combined_df.loc[
            (_combined_df["completion_tokens"] < 100)
            & (_combined_df["class"] == "title")
        ],
        x="completion_tokens",
        col="periodical_code",
